...
export const RACES = { ...CORE, ...RACES2, ...RACES3, ...PLACES };
...
// Compound-word mode (places): whole words joined with R.sep, each word checked
// on its own, so "Prancing Pony" passes where a 13-letter name would not.
function generateCompound(R, r) {
  for (let attempt = 0; attempt < 60; attempt++) {
    const syl = pick(r, R.syl);
    const parts = [pick(r, R.onset)];
    for (let i = 0; i < syl - 2; i++) parts.push(pick(r, R.mid));
    parts.push(pick(r, R.coda));
    const words = parts.map(w => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase());
    if (words[0].toLowerCase() === words[words.length - 1].toLowerCase()) continue;
    if (!words.every(w => w.length >= 3 && w.length <= 11 && pronounceable(w))) continue;
    const n = words.join(R.sep);
...
export function generateName(raceKey, gender, seedStr) {
  ...
  if (R.sep) return generateCompound(R, r);
  for (let attempt = 0; attempt < 60; attempt++) {
    const syl = pick(r, R.syl);
