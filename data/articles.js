...
export const GENRE_INTROS = {
  ...
  place: `Characters are only half of a campaign's names — the other half is painted on signboards. This hub gathers generators for the places your party actually spends its time: taverns and inns first, with guilds, cities, ships and kingdoms to follow. Each one is built on real naming conventions rather than shuffled words — tavern names follow the medieval adjective-plus-beast formula that produced every Prancing Pony and Green Dragon in fantasy — so the generated results read like they were already on a map.`,
