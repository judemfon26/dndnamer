...
export const GENRE_INTROS = {
  ...
  place: `Characters are only half of a campaign's names — the other half is painted on signboards. This hub gathers generators for the places and institutions your party actually deals with: taverns and inns, and the guilds that run the trade in them, with cities, ships and kingdoms to follow. Each one is built on real naming conventions rather than shuffled words — tavern names follow the medieval adjective-plus-beast formula that produced every Prancing Pony and Green Dragon in fantasy, and guild names follow the emblem-plus-charter grammar of the real livery companies — so the generated results read like they were already on a map.`,
